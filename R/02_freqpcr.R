...
#'
#' @slot report A matrix of the simultaneous parameter estimation result. The rows represent the parameters \code{P}, \code{K}, \code{targetScale} (\eqn{\delta_{T}}), the Cq measurement error as \code{sdMeasure} (\eqn{\sigma_{c}}), and \code{EPCR} (\eqn{\eta}).
...
#' Log-likelihood of getting Cq values under given parameter set.
#'
#' Internal function called from the optimizer (\code{\link[stats]{nlm}()}) running in \code{\link{freqpcr}()}. It defines the log-likelihood getting the two \eqn{\Delta}Cq values (differences in the four Cq measurements) provided the allele mixing ratio for each bulk sample is given together with other parameters. \code{\link{freqpcr_loglike}()} considers uncertainty in individual DNA yield while does not \code{\link{knownqpcr_loglike}()}. The function is vectorized over multiple (bulk) samples.
#' @param X Vector that stores the parameter values to be optimized via \code{\link{nlm}()}, \code{P} in logit scale and \code{K}, \code{targetScale}, \code{sdMeasure}, and \code{EPCR} in log scale.
...
#' @param para.fixed Named numeric vector that stores the fixed parameters, if specified by the user. By default (\code{NULL}), all the parameters are considered unknown. Unlike \code{X}, each element should be the raw parameter value in linear scale.
...
#' @description The function estimates the population allele frequency using the dataset of Cq values measured over \code{length(N)} bulk samples, each of which has the sample size of \code{N[i]} as the number of the chromosome sets (= the number of individuals for a haploid organism) included. \code{N[i]} can be 1, meaning every individual is analyzed separately.
#' \cr
...
#' @param target0 A numeric vector having the same length as N. In RED-\eqn{\Delta\Delta}Cq method, \code{target0[i]} signifies the measured Cq value of the ith bulk sample, which is treated with the universal primer set that amplifies both alleles, e.g. susceptible (S) and resistant (R) to a pesticide, on the target locus. In a general \eqn{\Delta\Delta}Cq analyses, \code{target0} is the Cq values measured for the control sample, a sample solution that contains 100\% R (the mutant genotype), which is amplified the universal primer set.
#' @param target1 A numeric vector having the same length as N. For each test sample with unknown allele-ratio (ith bulk sample), \code{target1[i]} is defined as the Cq value for the target locus amplified with the universal primer following the restriction enzyme digestion (In RED-\eqn{\Delta\Delta}Cq method), or the target locus amplified with the allele-specific primer set (in general \eqn{\Delta\Delta}Cq analyses).
#' @param housek0 A numeric vector having the same length as N. In RED-\eqn{\Delta\Delta}Cq method, it is the Cq values for the housekeeping gene of the test sample, which will be amplified regardless of genotypes. In a general \eqn{\Delta\Delta}Cq analyses, housek0 is defined for the control (100\% mutant) sample, which is also amplified with the primer set for the housekeeping gene.
#' @param housek1 A numeric vector having the same length as N. The Cq values of the test sample measured on the housekeeping gene after the restriction enzyme digestion (in RED-\eqn{\Delta\Delta}Cq method), or the test sample amplified on the housekeeping gene (in general \eqn{\Delta\Delta}Cq analyses).
#' @param P Population allele frequency from which the test samples are retrieved. Default is \code{NULL} and to be estimated. If the parameter size is known, it should be given as a numeric between 0 and 1.
#' @param K The gamma shape parameter of the individual DNA yield. Default is \code{NULL} and to be estimated. If known, give as a positive numeric.
#' @param targetScale (\eqn{\delta_{T}}) The relative template DNA amount of the target locus to the houskeeping locus. If known, give as a positive numeric.
#' @param sdMeasure (\eqn{\sigma_{c}}) The measurement error on each Cq value following Normal(0, \eqn{\sigma_{c}^2}). If known, give as a positive number.
#' @param EPCR (\eqn{\eta}) Amplification efficiency per PCR cycle. It should be given as a positive numeric if the parameter size is known. When \code{EPCR = 1}, template DNA doubles every cycle (\code{EPCR + 1 = 2}).
...
#' @param zeroAmount (In RED-\eqn{\Delta\Delta}Cq method) residual rate of restriction enzyme digestion, or (in general \eqn{\Delta\Delta}Cq analyses) small portion of the off-target allele amplified in the PCR. It needs to be always specified by the user as a number between 0 and 1, usually near 0.
#' @param beta Use the beta distribution for the approximation of allele frequency instead of the two gamma distributions for the DNA quantities of the alleles? Default is \code{TRUE}.
...
freqpcr <- function(N, target0, target1, housek0, housek1,
...
#    stop("Stop here.") # for debug
    ...
    if (class(success.nlm) == "try-error") {
        ...
        return( list(report=result, obj=list(iterations=NA), cal.time=cal.time) )
    } else {
        if(print.level > 0) {
